...
const SideRays = ({
  ...
  // IntersectionObserver to watch container visibility
  useEffect(() => {
  ...
  // WebGL context initializer (runs once when container is ready)
  useEffect(() => {
    if (!containerRef.current) return;
...
uniform float iSpread; uniform float iFlipX; uniform float iFlipY;
uniform float iTilt; uniform float iSaturation; uniform float iBlend;
uniform float iFalloff; uniform float iOpacity;
...
void main() {
  ...
  // Visibility pausing/resuming animation loop effect
  useEffect(() => {
    isVisibleRef.current = isVisible;

    if (isVisible) {
      if (!animationIdRef.current && rendererRef.current && meshRef.current) {
        const loop = (t: number) => {
          if (!rendererRef.current || !uniformsRef.current || !meshRef.current) return;
